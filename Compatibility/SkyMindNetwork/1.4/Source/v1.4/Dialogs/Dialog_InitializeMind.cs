...
namespace ATReforged
{
    public class Dialog_InitializeMind : Dialog_MessageBox
    {
            ...
            if (cloudPawns.Count() + networkedPawns.Count() > 0 && SMN_Utils.gameComp.networkedDevices.Count < SMN_Utils.gameComp.GetSkyMindNetworkSlots())
            {
                buttonAAction = delegate ()
                {
                    ...
                    foreach (Pawn pawn in cloudPawns)
                    {
                        ...
                        {
                            SMN_Utils.gameComp.AttemptSkyMindConnection(newIntelligence);
                    ...
                    if (opts.Count == 0)
                        Log.Error("[ATR/SMN] Initializing a mind via SkyMind attempted but no viable pawns were found!");
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            if (Widgets.ButtonText(new Rect(num5 * (num4 - 1) + 10f, inRect.height - 35f, width2, 35f), label) && InteractionDelayExpired)
            {
