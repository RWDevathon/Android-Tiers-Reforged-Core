    ...
    public class LovePartnerRelationUtility_Patch
    {
        ...
        public class RandomSelectionWeight_Patch
        {
            ...
            public static void Listener(Pawn first, Pawn second, ref bool __result)
            {
                ...
                if (ATRCore_Utils.IsSurrogate(first) && LovePartnerRelationUtility.LovePartnerRelationExists(first.GetComp<CompSkyMindLink>().GetSurrogates().FirstOrFallback(), second))
                {
                    __result = true;
                }
                else if (ATRCore_Utils.IsSurrogate(second) && LovePartnerRelationUtility.LovePartnerRelationExists(first, second.GetComp<CompSkyMindLink>().GetSurrogates().FirstOrFallback()))
                {
                    __result = true;
                }
            }
