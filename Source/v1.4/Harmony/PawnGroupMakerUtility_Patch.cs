...

    {
        ...
        public class GeneratePawns_Patch
        {
            ...
            public static void Listener(PawnGroupMakerParms parms, bool warnOnZeroResults, ref IEnumerable<Pawn> __result)
            {
                ...
                // Only factions with the faction extension can have surrogates.
                ATR_FactionExtension factionExtension = parms.faction.def.GetModExtension<ATR_FactionExtension>();
                ...
                try
                {
                    ...
                    foreach (Pawn pawn in __result)
                    {
                        ...
                        {
                            if (factionExtension.canUseOrganicSurrogates && !MHC_Utils.IsConsideredMechanical(pawn))
                            {
                    ...
                    foreach (Pawn selectedPawn in selectedPawns)
                    {
                        ...
                        ATR_PawnExtension androidExtension = selectedPawn.def.GetModExtension<ATR_PawnExtension>();
                        if (androidExtension?.canInherentlyUseSkyMind != true)
                        {
                            // If the extension specifies an implant, use that.
                            if (androidExtension.defaultReceiverImplant != null)
                            {
                                // If the extension specifies a transceiver implant, ensure the pawn does not have one.
                                if (androidExtension.defaultTransceiverImplant != null)
                                {
                                    Hediff hediff = selectedPawn.health.hediffSet.GetFirstHediffOfDef(androidExtension.defaultTransceiverImplant);
                                    if (hediff != null)
                                    {
                                        selectedPawn.health.RemoveHediff(hediff);
                                    }
                                }
                                selectedPawn.health.AddHediff(androidExtension.defaultReceiverImplant, selectedPawn.health.hediffSet.GetBrain());
                            }
                            // Androids use receiver cores as their receivers.
                            else if (androidExtension.isAndroid)
                            {
                            ...
                            else if (!MHC_Utils.IsConsideredMechanical(selectedPawn))
                            {
