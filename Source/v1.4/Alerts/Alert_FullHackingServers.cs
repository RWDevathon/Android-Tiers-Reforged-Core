...
namespace ATReforged
{
    public class Alert_FullHackingServers : Alert
    {
        ...
        {
            if (!ATReforged_Settings.playerCanHack || !ATReforged_Settings.receiveHackingAlert || Utils.gameComp.GetPointCapacity(ATR_ServerType.HackingServer) <= 0)
                return false;
            ...
            float points = Utils.gameComp.GetPoints(ATR_ServerType.HackingServer);
            if (points >= Utils.gameComp.GetPointCapacity(ATR_ServerType.HackingServer) * 0.9f && points >= Utils.gameComp.hackCostTimePenalty + 400)
            {
                return true;
            }
            return false;
